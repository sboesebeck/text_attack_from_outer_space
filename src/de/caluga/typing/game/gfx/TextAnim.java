...
public class TextAnim implements Obj {
    ...
    public TextAnim(String text, int x, int y, int r, int g, int b, int dur) {
        this(text, x, y, x, y, r, g, b, 15, 80, dur, 10);
    }

    public TextAnim(String text, int x, int y, int x2, int y2, int r, int g, int b, int startSize, int targetSize, int dur, int fadeOut) {
        this.text = text;
        this.x = x;
        this.x2 = x2;
        this.y = y;
    ...
    public void draw(Graphics2D g) {
        if (frame >= duration) {
            //fadeout
            int fadeoutFrame = frame - duration;
